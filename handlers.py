...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_html(
        ...
        f"Use /matches to chat with your mutual matches.\n"
        f"Use /cancel_chat to exit a conversation with a match.\n"
        f"Use /secret_crush to add someone special (either a bot user or external crush).\n"
        f"Use /help to see all available commands.\n\n"
        f"Join our channel @gbi_match_maker for updates and announcements!\n\n"
        f"Need assistance at any time? Use /help for a list of commands."
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_text = (
        ...
        "/cancel_chat - Exit the current chat conversation\n"
        ...
        "• You can only match with opposite-gender users\n"
        "• Ages must be between 18-30 years\n\n"
        
        "*Stay Updated:*\n"
        "• Join our channel @gbi_match_maker for announcements and tips!\n\n"
...
async def register_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_profile and user_profile.get("profile_complete"):
        ...
        await update.message.reply_text(
            ...
            "Need help? Type /help for all available commands."
        )
...
async def handle_match(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    result, liked_data = process_match_decision(user_id, match_id, action == "like")
    ...
    if result == "match":
        ...
        # Notify the person who was liked
        if liked_data and match_id:
            liker_profile = liked_data.get("sender_profile")
            if liker_profile:
                liker_name = liker_profile.get("name", "Someone")
                
                # Create a keyboard to view the profile of the person who liked them
                keyboard = [
                    ...
                    [
                        ...
                        InlineKeyboardButton("👍 Like Back", callback_data=f"like_{user_id}")
                    ]
                ]
                reply_markup = InlineKeyboardMarkup(keyboard)
                ...
                await context.bot.send_message(
                    chat_id=match_id,
                    text=f"❤️ {liker_name} liked your profile! Check out their profile and decide if you want to like them back.",
                    reply_markup=reply_markup
                )
...
async def handle_chat_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Check if we're in a chat with someone
        ...
        await update.message.reply_text(
            "I didn't understand that command. Use /help to see all available commands."
        )
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Handle view profile request (from like notifications)
    if data.startswith("view_profile_"):
        profile_id = int(data.split("_")[2])
        profile = get_user_profile(profile_id)
        
        if profile:
            profile_text = format_profile(profile, include_personal=False)
            
            # Create action keyboard
            keyboard = [
                [
                    InlineKeyboardButton("👎 Pass", callback_data=f"pass_{profile_id}"),
                    InlineKeyboardButton("👍 Like", callback_data=f"like_{profile_id}")
                ]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            # Create a custom update object to send the profile
            class CustomUpdate:
                def __init__(self, user_id):
                    self.message = type('obj', (object,), {
                        'chat_id': user_id,
            ...
            # Delete the original query message
    ...
    if action in ["like", "pass"]:
        ...
        if chat_history:
            ...
            await query.edit_message_text(
                text=f"{history_text}\n\n✅ You are now connected with your match. Simply type messages to chat."
            )
        else:
            # No chat history
            match_profile = get_user_profile(match_id)
            ...
            await query.edit_message_text(
                text=f"✅ You are now connected with {match_name}. This is the beginning of your conversation. "
...
async def cancel_chat_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Exit the current chat conversation."""
    user_id = update.effective_user.id
    
    if "chatting_with" in context.user_data:
        recipient_id = context.user_data["chatting_with"]
        # Get the match's profile to show their name
        match_profile = get_user_profile(recipient_id)
        match_name = match_profile.get("name", "your match") if match_profile else "your match"
        
        # Clear the chat state
        recipient_id = context.user_data.pop("chatting_with")
        
        await update.message.reply_text(
            f"You have exited the chat with {match_name}. Use /matches to see your other matches or start a new chat.\n\n"
            f"Need help? Type /help for all available commands."
        )
        
        # Show potential matches option
        keyboard = [
            [InlineKeyboardButton("Browse More Matches", callback_data="browse_matches")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await update.message.reply_text(
            "Would you like to browse more potential matches?",
            reply_markup=reply_markup
        )
    else:
        await update.message.reply_text(
            "You are not currently in a chat. Use /matches to see your matches and start chatting.\n\n"
            "Need help? Type /help for all available commands."
        )
...
async def handle_inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle inline queries for command execution without buttons."""
    ...
    user_profile = get_user_profile(user_id)
    is_registered = user_profile and user_profile.get("profile_complete", False)
    
    results = []
    
    # Command-specific results based on the query
    if query_text == 'profile':
        ...
        results.append(
            InlineQueryResultArticle(
                ...
                description='Display your current dating profile',
                input_message_content=InputTextMessageContent(
                    message_text='/profile'
                ),
                thumb_url="https://cdn-icons-png.flaticon.com/512/4086/4086679.png"
            )
        )
    elif query_text == 'match':
        # Match command
        results.append(
            InlineQueryResultArticle(
                id='match',
                ...
                thumb_url="https://cdn-icons-png.flaticon.com/512/2589/2589175.png"
            )
        )
    elif query_text == 'matches':
        # Matches command
        results.append(
            InlineQueryResultArticle(
                id='matches',
                title='Your Matches',
                description='View your current matches and chat with them',
                input_message_content=InputTextMessageContent(
                    message_text='/matches'
                ),
                thumb_url="https://cdn-icons-png.flaticon.com/512/1029/1029183.png"
            )
        )
    elif query_text == 'secret_crush' or query_text == 'secretcrush':
        # Secret crush command
        results.append(
            InlineQueryResultArticle(
                id='secret_crush',
                title='Add Secret Crush',
                description='Add someone as your secret crush',
                input_message_content=InputTextMessageContent(
                    message_text='/secret_crush'
                ),
                thumb_url="https://cdn-icons-png.flaticon.com/512/833/833472.png"
            )
        )
        ...
        results.append(
            InlineQueryResultArticle(
                ...
                thumb_url="https://cdn-icons-png.flaticon.com/512/1828/1828940.png"
            )
        )
    elif query_text == 'register':
        # Register command
        results.append(
            InlineQueryResultArticle(
                id='register',
                title='Register',
                description='Create your dating profile',
                input_message_content=InputTextMessageContent(
                    message_text='/register'
                ),
                thumb_url="https://cdn-icons-png.flaticon.com/512/3534/3534139.png"
            )
        )
    elif query_text == 'edit_profile':
        # Edit profile command
        results.append(
            InlineQueryResultArticle(
                id='edit_profile',
                title='Edit Profile',
                description='Update your dating profile',
                input_message_content=InputTextMessageContent(
                    message_text='/edit_profile'
                ),
                thumb_url="https://cdn-icons-png.flaticon.com/512/1159/1159633.png"
            )
        )
    elif query_text == 'cancel_chat':
        # Cancel chat command
        results.append(
            InlineQueryResultArticle(
                id='cancel_chat',
                title='Exit Current Chat',
                description='Leave the current conversation with a match',
                input_message_content=InputTextMessageContent(
                    message_text='/cancel_chat'
                ),
                thumb_url="https://cdn-icons-png.flaticon.com/512/1828/1828843.png"
            )
        )
        ...
        results.append(
            InlineQueryResultArticle(
                id='start',
                title='Start Bot',
                description='Get started with Ethiopian University Dating Bot',
                input_message_content=InputTextMessageContent(
                    message_text='/start'
                ),
                thumb_url="https://cdn-icons-png.flaticon.com/512/3855/3855249.png"
            )
        )
    else:
        # Personalized suggestions based on user status
        if not is_registered:
            # Not registered - prioritize registration
            results.extend([
                InlineQueryResultArticle(
                    id='register_help',
                    title='Register Your Profile',
                    description='Create your dating profile to get started',
                    input_message_content=InputTextMessageContent(
                        message_text='/register'
                    ),
                    thumb_url="https://cdn-icons-png.flaticon.com/512/3534/3534139.png"
                ...
                InlineQueryResultArticle(
                    ...
                    description='Learn how to use the Ethiopian University Dating Bot',
                    input_message_content=InputTextMessageContent(
                        message_text='/help'
                    ),
                    thumb_url="https://cdn-icons-png.flaticon.com/512/1828/1828940.png"
            ...
            results.extend([
                ...
                InlineQueryResultArticle(
                    id='matches_help',
                    title='Your Matches',
                    description='View and chat with your current matches',
                    input_message_content=InputTextMessageContent(
                        message_text='/matches'
                    ),
                    thumb_url="https://cdn-icons-png.flaticon.com/512/1029/1029183.png"
    ...
    # Answer with results, caching for 300 seconds (5 minutes) for better performance
    # except for personalized results which shouldn't be cached
    cache_time = 0 if not query_text else 300
